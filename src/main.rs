...
use crate::model::{BgAndNote, ChartType, Directions, NodeOrEdge, Options};
...
fn process_directives(line: &str, options: &mut Options) -> YumlResult<()> {
...
fn build_dot_header(is_dark: bool) -> String {
...
fn record_name(label: &str) -> &str {
    label.split('|').next().map(|l| l.trim()).unwrap_or_default()
}

fn serialize_dot(node_or_edge: NodeOrEdge) -> YumlResult<String> {
    match node_or_edge {
...
fn serialize_dot_elements(mut elements: Vec<NodeOrEdge>) -> YumlResult<String> {
...
fn add_bar_facet(elements: &mut [NodeOrEdge], name: &str) -> Option<String> {
...
fn extract_bg_and_note(part: &str, allow_note: bool) -> BgAndNote {
...
fn split_yuml_expr(line: &str, separators: &str, escape: Option<char>) -> YumlResult<Vec<String>> {
